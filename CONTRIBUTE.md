...
When running locally via `uvx` directly, the file is expected in your home directory `~/.netapp`. However, when using containers, it should be passed securely via a Docker Compose secret.

1. **Create the `.netapp` file**:
   - Open a terminal or file explorer.
   - Create a new file named `.netapp` (either in `~` if running via `uvx`, or in the project root if running via containers).
